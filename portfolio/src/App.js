...
const experiences = [
  {
    ...
    color: palette.blue,
    highlights: [
      "Spearheaded a booking engine from zero to production (~26K LOC, NestJS/TypeScript) sustaining 4,200+ concurrent sessions at sub-300ms p95 for a $1B+ entertainment mega-project across 3 markets",
      "Exposed a unified GraphQL/Apollo API layer and consolidated fragmented REST endpoints for 3 client applications",
      "Unified 5 external APIs (HyperPay, Magento, AEM, Visit Planner, Digital ID) into a single transactional Saga workflow, eliminating orphaned-payment bugs leaking ~$12K/month",
      "Resolved cart reservation race conditions with Redis SETNX distributed locking and PostgreSQL row-level versioning, reducing booking conflicts from ~340/week to zero at 1,800+ req/min",
      "Engineered Redis, PostgreSQL materialized view, and CDN caching layers, cutting average API response time from 820ms to 285ms",
      "Established a 10-stage GitLab CI/CD pipeline with Semgrep SAST, Invicti DAST, Trivy, Cypress, Jest, Helm-managed canary Kubernetes deployments, and zero failed releases across 94 deployments",
    ],
    stack: ["NestJS", "TypeScript", "GraphQL", "Apollo", "PostgreSQL", "Redis", "Kubernetes", "GitLab CI", "Saga Pattern"],
  },
  {
    company: "PwC",
    ...
    color: palette.cyan,
    highlights: [
      "Single-handedly delivered the backend API layer (NestJS, 47 endpoints, 18 entities) for an enterprise platform used daily by 2,200+ consultants, replacing a legacy PHP system",
      "Implemented enterprise authentication and authorization with Auth0, Okta, SCIM provisioning, and RBAC policies across 6 organizational roles",
      "Audited and optimized a 5.2M-row PostgreSQL dataset with partial and covering indexes plus Elasticsearch search, reducing p95 query time from 8.4s to 1.2s",
      "Developed a Salesforce CRM sync pipeline with idempotent upserts, conflict resolution, and exponential backoff, processing 14,000+ daily events at 99.97% success",
      "Introduced Terraform IaC and GitHub Actions CI/CD, reducing GCP environment setup from 2 business days to 12 minutes",
    ],
    stack: ["NestJS", "PostgreSQL", "Elasticsearch", "Auth0", "Okta", "Salesforce", "Terraform", "GCP Cloud Run", "GitHub Actions"],
  },
  {
    ...
    color: palette.green,
    highlights: [
      "Inherited a service layer with 340+ open bug tickets and p95 above 2.8s; profiled N+1 patterns, memory leaks, and pool exhaustion, then adopted CQRS to reduce p95 to 680ms",
      "Created a GCP Pub/Sub, Cloud Functions, Firestore, FCM, Twilio, and SendGrid notification engine delivering 1.2M+ events/month with 99.95% reliability",
      "Designed Redis write-through, cache-aside, and pub/sub invalidation strategies across 4 instances, cutting PostgreSQL read traffic by 35%",
      "Rebuilt the customer portal with React 18, Tailwind CSS, Redux, Context API feature flags, SSR via Next.js, and code splitting, improving Lighthouse performance from 54 to 91",
      "Led code reviews for 5 engineers and mentored 2 juniors, reducing production bug escape rate from 12/month to 4/month within one quarter",
    ],
    stack: ["Express", "Node.js", "CQRS", "GCP Pub/Sub", "Cloud Functions", "Firestore", "Redis", "React", "Next.js", "Tailwind"],
  },
  {
    company: "Wellnite",
    ...
    highlights: [
      "Developed real-time therapy session infrastructure for 47,000+ daily active users through a horizontally scalable Express WebSocket/WebRTC gateway with 99.99% uptime",
      "Replaced a fragile cron system losing ~6,500 jobs/month with Redis/BullMQ queues processing 108,000+ daily jobs at 99.97% completion",
      "Built a resilient integration layer for Stripe, SendGrid, Twilio, Firebase, GCP Vertex AI, and Sentry using adapters, circuit breakers, and graceful degradation",
      "Operated MongoDB replica sets for 53,000+ patient profiles and 1.8M+ session records, adding Atlas, Grafana, OpenTelemetry, and PagerDuty observability",
    ],
    stack: ["Express", "WebSocket", "WebRTC", "AWS EC2", "API Gateway", "SQS", "Redis", "BullMQ", "MongoDB", "GCP Vertex AI"],
  },
  {
    company: "Kuyua",
    role: "Senior Full Stack Engineer",
    ...
    color: palette.amber,
    highlights: [
      "Led a zero-downtime migration from monolithic Django to NestJS via the Strangler Fig pattern, improving p95 from 1.8s to 680ms, reducing infrastructure cost by 30%, and tripling feature velocity",
      "Restructured PostgreSQL for 1.1M+ ESG records with quarterly partitioning, 17 composite indexes, materialized views, PgBouncer, and Elasticsearch search",
      "Streamlined ingestion of 520,000+ monthly records from 8 third-party APIs using JSON Schema validation, z-score anomaly detection, content-hash deduplication, and dead-letter routing",
      "Built ESG analytics dashboards in React 18 with Tailwind CSS and GitHub Actions pipelines, using Claude Code and Cursor to reduce dashboard module implementation time by ~30%",
    ],
    stack: ["Django", "NestJS", "PostgreSQL", "PgBouncer", "Elasticsearch", "React", "Tailwind", "GitHub Actions", "Strangler Fig"],
  },
  {
    ...
    color: palette.blue,
    highlights: [
      "Conceived a FinTech bookkeeping SaaS from initial design to 10,400+ active business accounts processing 108,000+ monthly transactions",
      "Implemented ACID-compliant double-entry accounting across PostgreSQL and MySQL with 94% auto-match bank reconciliation, immutable audit trails, DDD, and CQRS",
      "Built an event-driven microservices backbone with RabbitMQ quorum queues, Kafka, and AWS SQS processing 3.2M+ monthly financial records with 99.98% reliability and zero data loss across 12 months",
      "Deployed services on AWS Lambda, EC2, S3, and API Gateway with Helm charts and GitOps workflows across dev, staging, and production",
      "Reduced platform downtime from 14 hours/month to under 45 minutes/month via Docker blue/green deployments, automated rollback, Prometheus/Grafana SLO alerting, and PagerDuty",
    ],
    stack: ["PostgreSQL", "MySQL", "DDD", "CQRS", "RabbitMQ", "Kafka", "AWS SQS", "Lambda", "Docker", "Prometheus", "Grafana"],
  },
];
